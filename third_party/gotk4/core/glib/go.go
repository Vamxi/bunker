...
func Overrides[OverridesT any](obj Objector) OverridesT {
	...
	if subclass, ok := knownTypes[rtype]; ok {
		...
		if ok && topt.override != nil {
			...
			// bunker patch: an ancestor's overrides (e.g. ObjectOverrides
			// requested during init_instance of a Widget subclass) were not
			// supplied; that means "not overridden", not a programming error.
		}
...
func OverridesFromObj[OverridesT any](obj *Object) OverridesT {
	...
	if subclass, ok := knownGTypes[gtype]; ok {
		topt, ok := subclass.opts.typeOpts[subclass.parentType]
		if ok && topt.override != nil {
			// bunker patch: hand the override factory the Go subclass
			// instance, not the bare *Object wrapper, so WithOverrides'
			// obj.(T) assertion yields the real receiver instead of nil.
			var target Objector = obj
			if inst, ok := privateFromInstance(unsafe.Pointer(obj.Native())).instance().(Objector); ok {
				target = inst
			}
			overrides, ok := topt.override(target).(OverridesT)
			if ok {
				return overrides
			}
			// bunker patch: ancestor overrides not supplied = not overridden.
		}
